...
$\frac{dx_1}{dt} = f_1x_1 (1 - \Sigma x_i) - Dx_1 + r x_2 + r x_3$ 

$\frac{dx_2}{dt} = f_2x_2 (1 - \Sigma x_i) - Dx_2 - (r + d + \eta)x_2 + r x_4 + r x_5$

$\frac{dx_3}{dt} = f_3x_3 (1 - \Sigma x_i) - Dx_3 + \eta x_2 - (r + d + \eta)x_3 + r x_5 + r x_6$

$\frac{dx_4}{dt} = f_4x_4 (1 - \Sigma x_i) - Dx_4 + d x_2 - (r + \eta)x_4$

$\frac{dx_5}{dt} = f_5x_5 (1 - \Sigma x_i) - Dx_5 + \eta x_3 + \eta x_4 - (2r + \eta)x_5$

$\frac{dx_6}{dt} = f_6x_6 (1 - \Sigma x_i) - Dx_6 + d x_3 + \eta x_5 - r x_6$
...
function dynamics!(du,u,p,t)	
    	...
    	du[1] = dx1 = f1*x1*(1 - xtotal) - D*x1 + r*x2 + r*x3
    	du[2] = dx2 = f2*x2*(1 - xtotal) - D*x2 - (r + d + η)*x2 + r*x4 + r*x5
		du[3] = dx3 = f3*x3*(1 - xtotal) - D*x3 + η*x2 - (r + d + η)*x3 + r*x5 + r*x6
		du[4] = dx4 = f4*x4*(1 - xtotal) - D*x4 + d*x2 - (r+η)*x4
		du[5] = dx5 = f5*x5*(1 - xtotal) - D*x5 + η*x3 + η*x4 - (2r+η)*x5
		du[6] = dx6 = f6*x5*(1 - xtotal) - D*x6 + d*x3 + η*x5 - r*x6
end
...
function dynamics2!(du,u,p,t)	
    	...
    	du[1] = dx1 = f1*x1*(1 - xtotal) - D*x1 + D*(r*x2 + r*x3)
    	du[2] = dx2 = f2*x2*(1 - xtotal) - D*x2 + D*(-(r + d + η)*x2 + r*x4 + r*x5)
		du[3] = dx3 = f3*x3*(1 - xtotal) - D*x3 + D*(η*x2 - (r + d + η)*x3 + r*x5 + r*x6)
		du[4] = dx4 = f4*x4*(1 - xtotal) - D*x4 + D*(d*x2 - (r+η)*x4)
		du[5] = dx5 = f5*x5*(1 - xtotal) - D*x5 + D*(η*x3 + η*x4 - (2r+η)*x5)
		du[6] = dx6 = f6*x5*(1 - xtotal) - D*x6 + D*(d*x3 + η*x5 - r*x6)
end

# ╔═╡ 27d302fe-1237-11eb-0166-1bf9048405e7
begin	
	## initial conditions
	x1, x2, x3, x4, x5, x6 = 0.0, 0.9, 0.0, 0.0, 0.0, 0
	u₀ = [x1, x2, x3, x4, x5, x6]
	## time interval
	tspan = (0.0,10000.0) 
end
...
begin 
	...
	r = 0.001#0.0134 ## from Tomanek et al. (2020)
...
antibiotic_prob = ODEProblem(dynamics2!, u₀, tspan, antibiotic_treatment);	

# ╔═╡ 69daf25e-124b-11eb-1fd1-7bb52f61b420
antibiotic_sol = solve(antibiotic_prob);

# ╔═╡ ec31ed3c-17c7-11eb-0700-6dae1e0fa0ad
pulse_antibiotic_prob = ODEProblem(dynamics2!, u₀, tspan, pulse_antibiotic_treatment);
