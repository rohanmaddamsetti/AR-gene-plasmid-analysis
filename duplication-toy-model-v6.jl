...
load("../results/AR-gene-duplication/diagrams/toy-model-v0.5.jpg")
...
There are six subpopulations of bacteria. Each cell contains a chromosome and a multi-copy plasmid. Each chromosome and plasmid may contain an antibiotic resistance gene (ARG). An ARG on a chromosome is shown as a large black bar, and an ARG on the plasmid is shown as a small black bar.

We are interested in the dynamics of the six subpopulations due to growth and mutation (duplication, loss, and transfer dynamics of the ARG). I roughly follow the modeling framework used by Lopatkin et al. (2017) "Persistence and reversal of plasmid-mediated antibiotic resistance", and described in that paper's Supplementary Information.
...
$\frac{dx_i}{dt} = f_i((1-q_i)x_i + \Sigma q_jx_j)(1 - \Sigma x_i) - Dx_i$ The first first term reflects logistic growth at a rate $f_i$ when carrying capacity has not been reached, and the second term reflects constant dilution due to a fixed outflow rate. The $q$ terms represent mutation dynamics due to gene duplication, loss, and transposition. The $q_i$ terms represent transitions to other states, and the $q_j$ terms represent transitions from other states.


**Model Equations**

$\frac{dx_1}{dt} = f_1(x_1 + r x_2 + r x_3)(1 - \Sigma x_i) - Dx_1$ 

$\frac{dx_2}{dt} = f_2((1 - r - d - \eta)x_2 + r x_4 + r x_5)(1-\Sigma x_i) - Dx_2$

$\frac{dx_3}{dt} = f_3((1 - r - d - \eta)x_3 + \eta x_2 + r x_5 + r x_6)(1 - \Sigma x_i) - Dx_3$

$\frac{dx_4}{dt} = f_4((1 - r - \eta)x_4 + d x_2)(1 - \Sigma x_i) - Dx_4$

$\frac{dx_5}{dt} = f_5((1 - 2r - \eta)x_5 + \eta x_3 + \eta x_4)(1 - \Sigma x_i) - Dx_5$

$\frac{dx_6}{dt} = f_6((1 - r)x_6 + d x_3 + \eta x_5)(1 - \Sigma x_i) - Dx_6$

**Growth functions**

$f_i = \frac{K_i}{K_i + A}$ where $A$ is antibiotic concentration and $K_i$ is the concentration of antibiotic that reduces growth by 50%. 

$f_j = (1-c) \frac{K_j}{K_j + A}$ where $(1-c)$ is the cost of expressing the ARG.

$f_k = (1-c)^2 \frac{K_k}{K_k + A}$ where $(1-c)^2$ is the cost of expressing 1 ARG on the plasmid, and an additional copy elsewhere in the genome.
...
# ╔═╡ 3d75339a-284c-11eb-0de5-43988452b8ef
function dynamics!(du,u,p,t)	
    	...
    	du[1] = dx1 = f1*(x1 + r*x2 + r*x3)*(1 - xtotal) - D*x1
    	du[2] = dx2 = f2*((1 - r + d + η)*x2 + r*x4 + r*x5) * (1 - xtotal) - D*x2 
		du[3] = dx3 = f3*((1 - r + d + η)*x3 +η*x2 + r*x5 + r*x6) * (1 - xtotal) - D*x3
		du[4] = dx4 = f4*((1 - r - η)*x4 + d*x2) * (1 - xtotal) - D*x4 
		du[5] = dx5 = f5*((1 - 2r - η)*x5 + η*x3 + η*x4)*(1 - xtotal) - D*x5 
		du[6] = dx6 = f6*((1 - r)*x6 + d*x3 + η*x5)*(1 - xtotal) - D*x6
end

# ╔═╡ 27d302fe-1237-11eb-0166-1bf9048405e7
begin	
	## initial conditions
	x1, x2, x3, x4, x5, x6 = 0.01, 0.01, 0.01, 0.01, 0.01, 0.01
	u₀ = [x1, x2, x3, x4, x5, x6]
	## time interval
	tspan = (0.0,10000.0) 
end
...
@bind DilutionRate Slider(0:0.0001:1, show_value=true)
...
savefig(p3, "../results/AR-gene-duplication/toy-model-dynamics-v0.6.pdf")

# ╔═╡ f441a87c-2855-11eb-12fe-d16c7a1c42e4
md""" __Duplication Index calculation__

DI  (duplication index) = $x_6 + x_4 + x_5$, that is, the fraction of the gene that will be duplicated (for a certain duplication cost).

DI ~ duplication cost (c ) and rate of duplication and transfer (d and eta)

Also consider the following alternative metrics:

$x_3 + x_5 + x_6$ fraction of populations containing the gene in the plasmid

$(x_5+x_6)/(x_4+x_5+x_6)$  fraction of duplicated genes in the plasmid


"""

# ╔═╡ fc4df2b4-2855-11eb-1735-d190a16ab2e2
begin
